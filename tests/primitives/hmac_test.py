...
def test_hmac_vector_1():
    ...
    key = b"\x0B" * 20
    data = b"Hi There"
    assert HMAC(key).generate(data) == bytes.fromhex(
...
def test_hmac_vector5():
    ...
    assert HMAC(key).generate(data)[:16] == bytes.fromhex("a3b6167473100ee06e0c796c2955552b")
...
def test_hmac_verify():
    ...
    mac = HMAC(key).generate(data)

    invalid_key = b"A different key"
    invalid_data = b"This is different data."
    invalid_mac = b"\xAA" * 32

    HMAC(key).verify(data, mac)
    assert HMAC(key).is_valid(data, mac)

    with raises(InvalidSignature):
        HMAC(key).verify(data, invalid_mac)
    with raises(InvalidSignature):
        HMAC(key).verify(invalid_data, mac)
    with raises(InvalidSignature):
        HMAC(invalid_key).verify(data, mac)

    assert not HMAC(key).is_valid(data, invalid_mac)
    assert not HMAC(key).is_valid(invalid_data, mac)
    assert not HMAC(invalid_key).is_valid(data, mac)
